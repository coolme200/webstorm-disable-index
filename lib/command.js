...
module.exports = class Command {
  * run(cwd) {
    this.cwd = cwd;
    this.pkgInfo = require(path.join(this.cwd, 'package.json'));
    this.appName = this.pkgInfo.name;
    this.config = objectPath.get(this.pkgInfo, 'config.webstorm-auto-config', {
      index: [],
    });
  ...
  * initApp() {
    ...
    [ '.tmp', 'temp', 'tmp', 'node_modules', 'coverage', 'logs' ].forEach(key => excludeFolderNode.push({ url: `file://$MODULE_DIR$/${key}` }));

    this.config.index.forEach(key => {
      componentNode.content.push({ url: `file://$MODULE_DIR$/node_modules/${key}` });
      excludeFolderNode.push({ url: `file://$MODULE_DIR$/node_modules/${key}/node_modules` });
    });
  ...
  * writeFile(fileName, content) {
    ...
    console.log('[webstorm-disable-index] write to', filePath);
