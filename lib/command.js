...
module.exports = class Command {
  * run(cwd) {
    ...
    // MULTI CONFIG BEGIN
    let configs = [this.pkgInfo];
    if(this.config.modules){
      for(let moduleName of this.config.modules){
        let pckInfo = require(path.join(this.cwd, moduleName, 'package.json'));
        pckInfo.____webstorm_disable_index_sub_module = moduleName;
        configs.push(pckInfo);
      }
    }

    for(let pckInfo of configs){
      pckInfo.config = pckInfo.config || {};
      pckInfo.config.webstorm = pckInfo.config.webstorm || {};
      yield this.initApp({
        indexArray: pckInfo.config.webstorm.index || [],
        subModule: pckInfo.____webstorm_disable_index_sub_module
      });
    }
  }

  * initApp({subModule, indexArray}) {
    ...
    let BASE_PATH = 'file://$MODULE_DIR$';
    subModule && (BASE_PATH = `${BASE_PATH}/${subModule}`);

    // console.log(contentNode.excludeFolder);
    const excludeFolderNode = contentNode.excludeFolder = [].concat(contentNode.excludeFolder || []);
    [ '.tmp', 'temp', 'tmp', 'node_modules', 'coverage', 'logs' ].forEach(key => excludeFolderNode.push({ url: `${BASE_PATH}/${key}` }));

    // console.log(excludeFolderNode);

    indexArray.forEach(key => {
      componentNode.content.push({ url: `${BASE_PATH}/node_modules/${key}` });
      excludeFolderNode.push({ url: `${BASE_PATH}/node_modules/${key}/node_modules` });
    });
